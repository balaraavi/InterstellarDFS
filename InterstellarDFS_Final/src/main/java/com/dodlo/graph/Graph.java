	...
	private final Map<String, Vertex> graph; // mapping of vertex names to Vertex objects, built from a set of Edges
	  ...
	  public static class Vertex implements Comparable<Vertex>{
		...
		
		{
			...
			{
				path =this.previous.printPath();
				distance = String.format("%.2f", this.dist);
				path = path +"->"+this.name+"("+ distance +")";
	   ...
	   public String printPath(String endName) {
	      ...
	      if (!graph.containsKey(endName)) {
	    	  path = "Graph doesn't contain end vertex"+endName;
	         System.err.printf("Graph doesn't contain end vertex \"%s\"\n", endName);
	         return path;
	      }
	 
	     path = graph.get(endName).printPath();
